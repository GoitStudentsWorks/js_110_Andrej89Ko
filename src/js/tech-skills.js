...
const tech = [
  {
    name: 'HTML/CSS',
    link: 'https://www.edu.goit.global/uk/learn/24554709/17039071/17039078/training',
  },
  {
    name: 'JavaScript',
...
const techSkillsUpper = wrap.querySelector(
  TECH_SKILLS.CSS_SELECTOR.TECH_SKILLS_UPPER_ID
);
const techSkillsLower = wrap.querySelector(
  TECH_SKILLS.CSS_SELECTOR.TECH_SKILLS_LOWER_ID
);

const contentMarkup = (content, place = TECH_SKILLS.PLACE.UPPER) =>
  content.map(tech => {
    return `<svg class=${
      place === TECH_SKILLS.PLACE.UPPER
        ...
        : TECH_SKILLS.CSS_CLASS.LOWER_DOTS
          ...
          <a target="_blank" href=${tech.link}>
            <div class="item ${
              place === TECH_SKILLS.PLACE.UPPER
                ? TECH_SKILLS.CSS_CLASS.ITEM_UPPER
                : TECH_SKILLS.CSS_CLASS.ITEM_LOWER
            }">${tech.name}</div>
          </a>`;
  });
...
const fillLine = (
  line,
  lineContent,
  position = TECH_SKILLS.POSITION.BEFORE_END
) => {
...
const lowerLineContent = buildTechLine(
  contentMarkup(tech, TECH_SKILLS.PLACE.LOWER),
  true
);
